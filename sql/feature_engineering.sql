...
create or replace view kkbox.members_all as(
	select
	 ...
	 gender,
	 registered_via,
	 to_date(string(registration_init_time),'yyyyMMdd') as registration_dt
 from kkbox.members
);
...
create or replace view kkbox.transactions_all as(
  ...
  select *,
  to_timestamp(concat(string(date_part('year',expiration_dt)),'-',string(date_part('month', expiration_dt)),'-',"01"), 'yyyy-M-dd') as ts
  from t
);
...
create or replace table kkbox.user_logs_agg (
	with log_matrix as (
